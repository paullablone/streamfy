...
function Signup({ onSignup, onSwitchToLogin, onBrowseAsGuest }) {
  ...
  const benefits = [
    { icon: '🎥', title: 'Create Your Channel', desc: 'Start streaming in minutes' },
    { icon: '👥', title: 'Build Your Community', desc: 'Connect with viewers worldwide' },
    { icon: '🏆', title: 'Earn Rewards', desc: 'Level up and unlock badges' },
    { icon: '💰', title: 'Monetization', desc: 'Receive donations & sponsorships' },
    { icon: '📊', title: 'Analytics', desc: 'Track your growth & stats' },
    { icon: '🎮', title: 'Interactive Tools', desc: 'Polls, DJ, watch parties & more' }
  ];
  ...
  return (
    <div className="auth-container" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div className="auth-box" style={{ maxWidth: '900px', width: '100%', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', padding: '50px', margin: '20px' }}>
        {/* Left Side - Benefits (Hidden on mobile via CSS) */}
        <div style={{ color: 'white' }} className="desktop-only">
          ...
          <h2 style={{ fontSize: '28px', marginBottom: '15px', fontWeight: '800' }}>Join Streamfy</h2>
          <p style={{ fontSize: '16px', marginBottom: '30px', opacity: 0.9 }}>
            Create your account and start your streaming journey today
          </p>
          ...
          <div style={{ display: 'grid', gap: '15px' }}>
            {benefits.map((benefit, index) => (
              ...
              >
                ...
                <div>
                  <div style={{ fontWeight: '700', fontSize: '15px' }}>{benefit.title}</div>
                  <div style={{ fontSize: '13px', opacity: 0.8 }}>{benefit.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ background: 'white', padding: '40px', borderRadius: '20px', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', width: '100%' }}>
          ...
          <h3 style={{ fontSize: '24px', marginBottom: '10px', color: '#1a1a1a', fontWeight: '800' }}>Create Account</h3>
          <p style={{ color: '#666', marginBottom: '25px', fontSize: '14px' }}>Fill in your details to get started</p>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              <label style={{ color: '#1a1a1a', fontSize: '13px', fontWeight: '600' }}>Username</label>
              <input
                type="text"
                value={username}
            ...
            <div className="form-group">
              <label style={{ color: '#1a1a1a', fontSize: '13px', fontWeight: '600' }}>Confirm Password</label>
              <input
                type="password"
